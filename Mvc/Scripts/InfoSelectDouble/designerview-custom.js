(function () {
    console.log("designeeeeeeeeeeeer");
    ...
    angular.module('designer').controller('CustomCtrl', ['$scope', 'propertyService', function ($scope, propertyService) {
        ...
        // whenever librariesSelector.selectedItemId changes, bind the new value to the widget controller property 
        $scope.$watch(
            'pagesSelector.selectedItemId',
            function (newSelectedItemId, oldSelectedItemId) {
                if (newSelectedItemId !== oldSelectedItemId) {
                    if (newSelectedItemId) {
                        $scope.properties.LinkedPageId.PropertyValue = newSelectedItemId;
                    }
                }
            },
            true
        ...
        // whenever librariesSelector.selectedItem changes, bind the new value to the widget controller property
        $scope.$watch(
            'pagesSelector.selectedItem',
            function (newSelectedItem, oldSelectedItem) {
                if (newSelectedItem !== oldSelectedItem) {
                    if (newSelectedItem) {
                        $scope.properties.LinkedPage.PropertyValue = JSON.stringify(newSelectedItem);
                    }
                }
            },
            true
        ...
        propertyService.get()
            .then(function (data) {
                if (data) {
                    ...
                    selectedItemId = $scope.properties.LinkedPageId.PropertyValue || null;
                    $scope.pagesSelector.selectedItemId = selectedItemId;

                    selectedItem = $scope.properties.LinkedPage.PropertyValue || null;
                    $scope.pagesSelector.selectedItem = selectedItem;
